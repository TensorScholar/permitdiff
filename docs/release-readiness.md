...
- GitHub Release creation and PyPI publication are treated as a distributed transaction with no atomic commit across services. A partial release is not repaired by rebuilding or replacing artifacts under the same version. If one publication stage succeeds and a later stage fails, repair the failed external dependency and rerun only the failed publication stage against the retained immutable distributions when possible. If the exact artifacts are no longer recoverable, advance the version rather than recreate them.
